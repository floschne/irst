...
from models import ModelRanking, StudyType, RatingWithFocusSample


class RatingWithFocusStudyCoordinator(StudyCoordinatorBase):
    __singleton = None

    def __new__(cls, *args, **kwargs):
        if cls.__singleton is None:
            cls.__singleton = super(RatingWithFocusStudyCoordinator, cls).__new__(cls)
    ...
    def __init__(self):
        ...
        if getattr(self, 'num_top_k_imgs', None) is None:
            sub_conf = conf.study_initialization[StudyType.RATING_WITH_FOCUS]
            super().__init__(StudyType.RATING_WITH_FOCUS,
    ...
    def _generate_sample(self, mr: ModelRanking) -> RatingWithFocusSample:
        # get top-k from model ranking
        tk_imgs = mr.top_k_image_ids[0:self.num_top_k_imgs]

        # create the sample
        rs = RatingWithFocusSample(mr_id=mr.id,
                                   caption=mr.query,
                                   focus=mr.opts["focus"],
        ...
        RedisHandler().store_rating_with_focus_sample(rs)
